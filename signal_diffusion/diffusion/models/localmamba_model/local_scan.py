"""
Local scan operations for LocalMamba.

Wraps the Triton kernels and PyTorch fallbacks that implement the windowed
scan/reverse patterns (horizontal, vertical, w2, w7, flipped variants) used by
`MultiScanVSSM` before calling `selective_scan_fn`.
...
# PyTorch fallback implementations for local scan and reverse

def local_scan(x, w=7, H=14, W=14, flip=False, column_first=False):
    """Local windowed scan in LocalMamba (PyTorch fallback)
...
def local_scan_bchw(x, w=7, H=14, W=14, flip=False, column_first=False):
    """Local windowed scan in LocalMamba (BCHW format)
...
def local_reverse(x, w=7, H=14, W=14, flip=False, column_first=False):
    """Local windowed reverse in LocalMamba (PyTorch fallback)
