...
@flags.compile_wrap
def scale_for_cosine_sim_qkv(qkv, scale, eps) -> torch.Tensor:
    q, k, v = qkv.unbind(2)
...
class Linear(nn.Linear):
    def forward(self, input):
        flops.op(flops.op_linear, input.shape, self.weight.shape)
...
class LinearGEGLU(nn.Linear):
    ...
    def forward(self, input):
        flops.op(flops.op_linear, input.shape, self.weight.shape)
        return linear_geglu(input, self.weight, self.bias)
...
@flags.compile_wrap
def apply_rotary_emb(x, theta, conj=False) -> torch.Tensor:
    out_dtype = x.dtype
    ...
    def __init__(self, levels, mapping, in_channels, out_channels, patch_size, num_classes=0, mapping_cond_dim=0):
        ...
        for i, spec in enumerate(levels):
            def layer_factory(index: int) -> nn.Module:
                ...
                if isinstance(spec.self_attn, NeighborhoodAttentionSpec):
                ...
                if isinstance(spec.self_attn, ShiftedWindowAttentionSpec):
                ...
                if isinstance(spec.self_attn, NoAttentionSpec):
